...
function WorkingNames({
    ...
    const HandleSerachId = async (configID) => {
        const { data, error } = await SearchcalculationVeriables(configID, "searchByWorkingConfigid")
        if (data) {
    ...
    const HandleDeleteVaribale = async (id) => {
        setdisabled(true)
        const { data, error } = await DeleteVariableCalculation(id, "DeleteWorkingCalculation")
        if (data) {
            ...
            <div className="px-4 py-2 overflow-y-scroll h-[420px]">
                ...
                {VeriableList.length > 0 && Array.isArray(VeriableList) &&
                    VeriableList.map((item, index) => {
                        const bg = item?.VariableType && item?.VariableType == "new_time" ? "bg-[#DFB006]" : "bg-[#F6F6F6]"
                        return (
                            < div key={index} className={`grid mt-2 grid-cols-4 gap-[10px] ${bg} rounded-[5px] border border-black border-solid px-[10px] py-[5px] whitespace-nowrap overflow-hidden text-ellipsis `}>
