...
function VariableFunctions({ MaterialCalculationList, setMaterialCalculationList, VeriableList, totalReset }) {
    ...
    const SearchVariable = async (search, index) => {
        ...
        if (ID?.trim()?.length > 5 && ID?.trim()?.length < 9) {
            const { data, error } = await SearchMaterial_ConfigID(ID)
            if (data?.data?.length > 0) {
                ...
                toast("Variable is available. Changes cannot be made, only deletion is allowed.", {
                    icon: '⚠️',
                });
            }
    ...
    const SearchFunction = async (search, index) => {
        const ID = search.target.value;
        if (ID?.trim()?.length > 5 && ID?.trim()?.length < 9) {
            const { data, error } = await SearchFunction_ConfigID(ID)
    ...
    const CreateCalculationMaterial = async () => {
        const { data, error } = await CreateCalculation(MaterialCalculationList, "addDataToVariable", TestTottal)
        if (data) {
    ...
    return (
        <div className="py-[7px]  flex-1 flex-col h-full flex items-center justify-between shadow-md bg-white border-2 border-solid border-[#d9d9d9] rounded-[12px]">
