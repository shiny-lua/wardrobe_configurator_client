...
function NewWorking({ setVeriableList, AddTime }) {
    ...
    const AddVeriable = async () => {
        if (VeriableName?.trim() && VeriableName != "") {
            ...
            AddTime && formdata.append("VariableType", "new_time");
            try {
                const data = await post(`/api/v1/variables/CreateWorkingVariable`, formdata);
