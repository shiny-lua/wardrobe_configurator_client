...
function FeesNames({
    ...
    const HandleSerachId = async (configID) => {
        const { data, error } = await SearchcalculationVeriables(configID, "searchByFeesConfigid")
        if (data) {
    ...
    const HandleDeleteVaribale = async (id) => {
        setdisabled(true)
        const { data, error } = await DeleteVariableCalculation(id, "DeleteFeesCalculation")
        if (data) {
