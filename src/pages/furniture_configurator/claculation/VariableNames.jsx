...
function VeriableNames({
    ...
    const HandleSerachId = async (configID) => {
        const { data, error } = await SearchcalculationVeriables(configID, "searchByConfigid")
        if (data) {
    ...
    const HandleDeleteVaribale = async (id) => {
        setdisabled(true)
        const { data, error } = await DeleteVariableCalculation(id, "DeleteCalculation")
        if (data) {
            setdisabled(false)
            ...
            <div className="px-4 py-2 overflow-y-scroll h-[420px]">
                ...
                {VeriableList.length > 0 && Array.isArray(VeriableList) &&
                    VeriableList.map((item, index) => {
                        return (
                            <div key={index} className="grid mt-2 grid-cols-4 gap-[10px] bg-[#F6F6F6] rounded-[5px] border border-black border-solid px-[10px] py-[5px] whitespace-nowrap overflow-hidden text-ellipsis ">
