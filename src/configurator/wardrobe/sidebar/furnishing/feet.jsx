...
import { ReactComponent as RotateIcon } from "../../../../assets/icons/fa_rotate-left.svg";
...
export default function Feet() {
    ...
    doorAssets.map((door, index) => {
      if ( door.bottomAsset === "none" && door.isPlint === false) {
        ...
        const payload ={
          ...
          scale: [scale[0], scale[1]+Config.plate.plinthHeight/2+Config.plate.thickness*2, scale[2]],
          topAsset: topAsset,
  ...
  // const tab_data = [
  //   { label: "Mit Fuß", value: "with-feet" },
  //   { label: "Hängend", value: "hanging"},    
  //   { label: "Ohne Fuß", value: "without-feet" },
  // ]
  ...
  useEffect(() => {
    console.log(feets)
    let temp = []
    if (feets.value.withFeet) {
      temp.push({ label: "Mit Fuß", value: "with-feet" })
    }
    if (feets.value.hanging) {
      temp.push({ label: "Hängend", value: "hanging"})
    }
    if (feets.value.withOutFeet) {
      temp.push({ label: "Ohne Fuß", value: "without-feet" })
    }
    setTabData(temp)
