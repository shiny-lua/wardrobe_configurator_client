...
import { Plane, RoundedBox, Html } from "@react-three/drei"
...
import PlusIcon  from "/images/configurator/icons/plus-1.png"
import MinusIcon  from "/images/configurator/icons/minus.png"
...
const DrawerComponent = React.memo(function DrawerComponent({
  ...
  const handleBlur = useCallback((drawerHeight) => {
      ...
      >
        ...
        {showControl && drawerType === Config.furnishing.drawer.type.customDrawer && (
          <group>
            ...
            {console.log(drawerHeightValue)}
            {drawerHeightValue < 41 && (
              <group>
                <mesh
                  onPointerOver={() => {}}
                  onPointerOut={() => {}}
                >
                  ...
                  >
                      {!showDimentionControl? (
                        ...
                        <div 
                          className={` bg-[#b6b6b698] justify-center gap-1`}
                          style={{width: `${scale[0]*2.6}px`,  display: `${scale[0]<43 ? "" : "flex"}`, height: `${scale[0]<43 ? "auto" : "51px"}`}}
                          tabIndex={0}  // Makes this element focusable
                          onBlur={() => {
                            if (!event.currentTarget.contains(event.relatedTarget)) {
                              setShowDimensionControl(false);
                          ...
                          > 
                          <div className="w-[20px] flex items-center mx-auto"
                            onClick={() => {
                              if (Math.round(drawerHeightValue) > 10) {
                                setDrawerHeightValue(Number(drawerHeightValue - 1))
                                const drawerHeight = Number(drawerHeightValue - 1)
                                handleBlur(drawerHeight)
                              }
                            }}
                            >
                            <img src={MinusIcon} className="w-full cursor-pointer"/>
                          </div>
                          <div className="text-[14px] h-[22px] m-auto border border-[#36695C] rounded-[6px] px-1 bg-white"
                            style={{width: `${scale[0]<43 ? "65px" : "auto"}`}}
                            onBlur={() => {
                              handleBlur()
                            }} // Handles blur event
                          ...
                          <div className="w-[20px] flex items-center mx-auto"
                            onClick={() => {
                              if (Math.round(drawerHeightValue) < 40) {
                                setDrawerHeightValue(Number(drawerHeightValue + 1))
                                handleBlur(Number(drawerHeightValue - 1))
                              }
                            }}
                          >
