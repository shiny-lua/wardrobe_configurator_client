...
export const HandlesAccordianAfter = ({
  ...
  const Handlesubmit = async (values, { resetForm }) => {
    ...
    if (createDrawer) {
      const { data, error } = await CreateDrawer(values, "createothers")
      if (data) {
        resetForm()
      ...
      const { data, error } = await EditDrawer(values, id, "updateothers")
      if (data) {
        toast.success(data?.message)
        drawerlistUpdate(data?.data)
        setdisabled(false)
  ...
  const DeleteSelectedDrawer = async (id) => {
    setdisabled(true)
    const { data, error } = await DeleteDrawer(id, "deleteothers")
    if (data) {
      toast.success(data?.message)
      drawerlistUpdate(data?.data)
      setdisabled(false)
