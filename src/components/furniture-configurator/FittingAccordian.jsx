...
import { fittingvalidationSchema } from "../../Formik/FormikFunctions"
import {
  CreateDrawer,
  DeleteDrawer,
  EditDrawer,
...
export const HandlesAccordianAfter = ({
  ...
  const Handlesubmit = async (values, { resetForm }) => {
    ...
    if (createDrawer) {
      const { data, error } = await CreateDrawer(values, "createfitting")
      if (data) {
        resetForm()
      ...
      const { data, error } = await EditDrawer(values, id, "updatefitting")
      if (data) {
        toast.success(data?.message)
        drawerlistUpdate(data?.data)
        setdisabled(false)
  ...
  const DeleteSelectedDrawer = async (id) => {
    setdisabled(true)
    const { data, error } = await DeleteDrawer(id, "deletefitting")
    if (data) {
      toast.success(data?.message)
      drawerlistUpdate(data?.data)
      setdisabled(false)
  ...
  return (
    <Formik
      ...
      validationSchema={fittingvalidationSchema}
      enableReinitialize
    >
      {(formik) => (
