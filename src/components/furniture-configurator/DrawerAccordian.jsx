...
export const DrawerAccordianBefore = ({ handleToggle, viewdata }) => {
  const baseUrl = import.meta.env.VITE_BACKEND_URL_img
  const HandleEdit = (id) => {
...
export const DrawerAccordianAfter = ({
  handleToggle,
  drawerlistUpdate,
  createDrawer,
  viewdata,
  ...
  const Handlesubmit = async (values, { resetForm }) => {
    ...
    if (createDrawer) {
      const { data, error } = await CreateDrawer(values, "createDrawers")
      if (data) {
        resetForm()
      ...
      const { data, error } = await EditDrawer(values, id, "updateDrawers")
      if (data) {
        toast.success(data?.message)
        drawerlistUpdate(data?.data)
        setdisabled(false)
  ...
  const DeleteSelectedDrawer = async (id) => {
    setdisabled(true)
    const { data, error } = await DeleteDrawer(id, "deleteDrawers")
    if (data) {
      toast.success(data?.message)
      drawerlistUpdate(data?.data)
      setdisabled(false)
  ...
  const DrawerinitialValues = {
    ...
    plateMaterialId: viewdata?.plateMaterialId || "",
        ...
        <Form className="w-full pb-4">
          <div className="flex relative justify-between flex-wrap p-5 pr-16">
            ...
            <div className="flex flex-col items-center justify-between h-full">
              <div className="flex flex-row  mb-2 justify-between w-full gap-[25px]">
                <h1 className="font-[karla] text-[20px] font-semibold">Name</h1>
                <div className="w-[170px]">
                  <div className="relative">
                    <Field
          ...
          <div className="flex pr-16">
            <div className="flex items-center justify-between h-full m-auto">
              <div className="m-4">Drawer Description</div>
